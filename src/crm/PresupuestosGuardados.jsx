...
/* ─── Config ──────────────────────────────────────────────────────────────── */
const ESTADOS = ["Todos", "Borrador", "Enviado", "Convertido"];
const ESTADO_CFG = {
  Borrador:   { dot: "bg-gray-400",   badge: "bg-gray-100 text-gray-600",    label: "Borrador — pendiente de enviar" },
  Enviado:    { dot: "bg-blue-500",   badge: "bg-blue-50 text-blue-700",     label: "Enviado al cliente"             },
  Convertido: { dot: "bg-violet-500", badge: "bg-violet-50 text-violet-700", label: "Convertido en factura"          },
};
const PERIODS = [
...
/* ─── Mobile Card ─────────────────────────────────────────────────────────── */
function MobileCard({ doc, onEdit, onPdf, onConvertir, onDelete }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4 mx-4">
      ...
      <div className="px-5 pt-5 pb-4 flex items-start gap-4">
        <div className="w-12 h-12 rounded-2xl bg-blue-100 text-blue-700 font-black text-lg flex items-center justify-center shrink-0">
      ...
      <div className="mx-4 mb-4 bg-gray-50 rounded-2xl px-5 py-4">
        <p className="text-sm font-medium text-gray-500 mb-1">Importe total del presupuesto</p>
      ...
      <div className="px-4 pb-5 space-y-2">
        <button onClick={onEdit}
          className="w-full h-14 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-base font-bold rounded-xl transition-colors flex items-center justify-center gap-3 shadow-sm">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
            <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/>
            <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/>
          </svg>
          Abrir y editar este presupuesto
        </button>

        <button onClick={onPdf}
          className="w-full h-13 py-3.5 bg-gray-100 hover:bg-gray-200 active:bg-gray-300 text-gray-800 text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-3">
...
/* ─── Desktop Row ─────────────────────────────────────────────────────────── */
function DesktopRow({ doc, onEdit, onPdf, onConvertir, onDelete }) {
  const total  = docTotal(doc);
  const estado = doc.estado || "Borrador";
  const cfg    = ESTADO_CFG[estado] || ESTADO_CFG.Borrador;

  return (
    <tr className="border-b border-gray-100 last:border-0 hover:bg-blue-50/30 transition-colors">
...
/* ─── Main ────────────────────────────────────────────────────────────────── */
export default function PresupuestosGuardados() {
  ...
  const allYears = useMemo(() => {
    const s = new Set();
    presupuestos.forEach(d => { const dt = toDate(d.fecha); if (dt) s.add(dt.getFullYear()); });
  ...
  const filtered = useMemo(() => {
    ...
    return presupuestos
      .filter(d => {
        ...
        if (estadoFilter !== "Todos" && (d.estado || "Borrador") !== estadoFilter) return false;
  ...
  }, [presupuestos, busqueda, estadoFilter, yearFilter, periodFilter]);

  const totalActivo = useMemo(
    () => filtered.filter(d => d.estado !== "Convertido").reduce((s, d) => s + docTotal(d), 0),
    [filtered]
  );

  const handleEliminar  = async (id) => { await removePresupuesto(id); setConfirm(null); toast.success("Presupuesto eliminado"); };
  const handleConvertir = async (doc) => {
    await upsertBorrador({ ...doc, id: doc.id || crypto.randomUUID(), facturada: false, documentType: "factura", createdAt: doc.createdAt || Date.now() });
    toast.success("Listo — el presupuesto se ha convertido en borrador de factura");
  };
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      {confirm && (
        <ConfirmDialog
          message={`¿Seguro que quieres eliminar el presupuesto "${confirm.numero || "sin número"}"? Esta acción no se puede deshacer.`}
          ...
          <div>
            ...
            <p className="text-base text-gray-500 mt-1">
              {loading
                ...
                : `${filtered.length} presupuesto${filtered.length !== 1 ? "s" : ""} · Total: ${fmtEur(totalActivo)}`}
            </p>
          </div>
          <button onClick={() => navigate("/presupuestos")}
            className="flex items-center gap-2 px-5 py-3 bg-blue-600 hover:bg-blue-700 text-white text-base font-bold rounded-2xl transition-colors shadow-sm">
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-12 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-base placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 transition-all"
          />
          {busqueda && (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            <div className="w-20 h-20 bg-blue-50 rounded-3xl flex items-center justify-center mb-5">
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth={1.5} strokeLinecap="round">
                <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/>
                <polyline points="14 2 14 8 20 8"/>
            ...
            <p className="text-xl font-bold text-gray-800 mb-2">
              {presupuestos.length === 0 ? "Todavía no tienes presupuestos" : "No hay resultados"}
            </p>
            <p className="text-base text-gray-400 mb-6 max-w-xs leading-relaxed">
              {presupuestos.length === 0
                ? "Pulsa el botón de abajo para crear tu primer presupuesto"
                : "Prueba a cambiar los filtros o el texto de búsqueda"}
            </p>
            {presupuestos.length === 0 && (
              <button onClick={() => navigate("/presupuestos")}
                className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white text-base font-bold rounded-2xl transition-colors shadow-sm">
      ...
      <button onClick={() => navigate("/presupuestos")}
        className="md:hidden fixed bottom-20 right-4 flex items-center gap-2 px-5 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl shadow-lg z-40 transition-colors"
